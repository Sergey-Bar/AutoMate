import { forwardRef, useState, cloneElement, type ReactElement, type HTMLAttributes, isValidElement } from 'react';
...
export function Popover({ children }: PopoverProps) {
  ...
  const trigger = children.find(child => isValidElement(child) && child.type === PopoverTrigger) as ReactElement<React.ButtonHTMLAttributes<HTMLButtonElement>> | undefined;
  const content = children.find(child => isValidElement(child) && child.type === PopoverContent) as ReactElement<React.HTMLAttributes<HTMLDivElement>> | undefined;

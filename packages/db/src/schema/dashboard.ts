...
export const runs = pgTable(
  ...
  (t) => [
    uniqueIndex('runs_workspace_external_source_unique').on(t.workspaceId, t.source, t.externalId),
...
export const trends = pgTable(
  ...
  (t) => [primaryKey({ columns: [t.date, t.project, t.branch] })],
);
...
export const quarantine = pgTable(
  'quarantine',
  {
    ...
    // Fails closed: a newly quarantined test starts as `pending` (not excluded
    // from the pass rate) until a human resolves it. The previous default,
    // `approved`, meant a test entered the quarantine and was immediately
    // hidden from reporting with nobody deciding that.
  ...
  (table) => [
    // A time-to-fix is a claim that the problem was fixed; it cannot exist
...
export const failureClassifications = pgTable(
  ...
  (t) => [uniqueIndex('failure_classifications_fingerprint_run_unique').on(t.fingerprint, t.runId)],
);

// ─── test failure correlations ─────────────────────────────────────────────
// Source: test_failure_correlations table — source-file ↔ test failure correlation window
export const testFailureCorrelations = pgTable(
  'test_failure_correlations',
  {
    ...
    lastUpdatedRunId: uuid('last_updated_run_id').references(() => runs.id, {
      onDelete: 'cascade',
    }),
...
export const apiKeys = pgTable(
  'api_keys',
  {
    ...
    // Least privilege: a key created without a role gets `viewer`, not `admin`.
    // The old default meant a caller who simply omitted the role silently
    // received full administrative access. Granting `admin` is now an explicit
    // choice at the boundary.
    role: text('role', { enum: ['admin', 'editor', 'viewer', 'service'] })
      .notNull()
      .default('viewer'),
  ...
  (t) => [
    ...
    // A row that claims administrative authority must say so in a column, not
    // inherit it from a default.
    check('api_keys_admin_role_explicit_check', sql`t.role <> 'admin' or t.scopes is not null`),
  ],
);
...
export const agentSessions = pgTable(
  'agent_sessions',
  {
    ...
    status: text('status', { enum: ['active', 'closed', 'error'] })
      .notNull()
      .default('active'),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull(),
...
export const agentSessionRuns = pgTable(
  'agent_session_runs',
  {
...
export const agentSessionFiles = pgTable(
  'agent_session_files',
  {
    id: text('id').primaryKey(),
    sessionId: text('session_id')
...
export const generatedTestSuggestions = pgTable(
  'generated_test_suggestions',
  {
    id: text('id').primaryKey(),
    ...
    status: text('status', { enum: ['draft', 'accepted', 'rejected', 'edited'] })
      .notNull()
      .default('draft'),
    ...
    createdAt: timestamp('created_at', { withTimezone: true }).notNull(),
    // `defaultNow()`, not an epoch-0 literal cast through `unknown`. A row that
    // defaulted to 1970 looked 56 years stale to every "last updated" query,
    // which is indistinguishable from a suggestion nobody ever touched.
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => [
...
export const repairAttempts = pgTable(
  ...
  (t) => [
    ...
    uniqueIndex('repair_attempts_session_id_attempt_number_unique').on(
      t.sessionId,
      t.attemptNumber,
    ),
  ],
...
export const auditEvents = pgTable(
  'audit_events',
  {
    ...
    actorType: text('actor_type', { enum: ['user', 'system', 'service'] })
      .notNull()
      .default('user'),

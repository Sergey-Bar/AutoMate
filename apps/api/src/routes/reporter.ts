...
const UploadedTestSchema = z.object({
...
async function parseReporterUpload(c: Context): Promise<ReporterUploadPayload> {
  ...
  if (contentType.includes('multipart/form-data')) {
    ...
    const artifactType = String(form.get('artifactType') ?? '').trim().toLowerCase();
...
function convertPlaywrightJsonToUpload(runIdFromField: string, report: Record<string, unknown>): ReporterUploadPayload {
  const runId = runIdFromField || (typeof report['runId'] === 'string' ? report['runId'] : 'uploaded-run');
    ...
    for (const spec of specs) {
      ...
      if (typeof primary === 'object' && primary !== null) {
        ...
        if (typeof lastResult === 'object' && lastResult !== null) {
          ...
          const rawStatus = typeof resultObject['status'] === 'string' ? resultObject['status'] : 'queued';
          status = mapPlaywrightStatus(rawStatus);
          durationMs = typeof resultObject['duration'] === 'number' ? resultObject['duration'] : null;
        }
      }
...
export function createReporterRoutes(
  ...
  app.post('/api/v1/reporter/events', async (c) => {
    ...
    if (isLegacyShape(raw)) {
      ...
      if (!result.success) {
        return c.json(
          { error: 'Invalid versioned reporter event', details: result.error.flatten().fieldErrors },
          400,
        );
      }
  ...
  app.post('/api/v1/reporter/upload', async (c) => {
    ...
    try {
      ...
      return c.json(
        { error: 'Invalid reporter upload payload' },
        400,
      );
    }

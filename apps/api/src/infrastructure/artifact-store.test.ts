...
describe('LocalArtifactStore', () => {
  it('writes, verifies, and reads a relative artifact', async () => {
    ...
    try {
      ...
      const artifact = await store.put({ key: 'runs/run-1/report.json', bytes: new TextEncoder().encode('{}'), contentType: 'application/json' });
  ...
  it('rejects traversal keys', async () => {
    ...
    await expect(store.put({ key: '../secret', bytes: new Uint8Array(), contentType: 'text/plain' })).rejects.toThrow();
  });
});

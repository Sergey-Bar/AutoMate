...
describe('Health routes', () => {
  ...
  it('GET /api/v1/a11y/audit returns 200 with schema-compatible payload', async () => {
    const res = await app.request('/api/v1/a11y/audit');
    expect(res.status).toBe(200);
    ...
    expect(typeof body.pagesScanned).toBe('number');
...
describe('Auth matrix', () => {
  ...
  describe('valid auth grants access to sensitive routes', () => {
    ...
    it('GET /api/v1/dashboard/runs returns 200 with valid Bearer token', async () => {
      const res = await app.request('/api/v1/runs', {
        headers: { Authorization: `Bearer ${TEST_KEY}` },
      });
      expect(res.status).toBe(200);
    ...
    it('GET /api/v1/orchestrator/conversations returns 200 with valid Bearer token', async () => {
      const res = await app.request('/api/v1/orchestrator/conversations', {
        headers: { Authorization: `Bearer ${TEST_KEY}` },
      });
      expect(res.status).toBe(200);
    ...
    it('GET /api/v1/vault/credentials returns 200 with valid Bearer token', async () => {
      const res = await app.request('/api/v1/vault/credentials', {
        headers: { Authorization: `Bearer ${TEST_KEY}` },
      });
      expect(res.status).toBe(200);
    ...
    it('GET /api/v1/connectors returns 200 with valid Bearer token', async () => {
      const res = await app.request('/api/v1/connectors', {
        headers: { Authorization: `Bearer ${TEST_KEY}` },
      });
      expect(res.status).toBe(200);
    ...
    it('POST /api/v1/agents/browser/generate returns 200 with valid Bearer token', async () => {
      const res = await app.request('/api/v1/agents/browser/generate', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ prompt: 'generate a test for the login page' }),
      });
      expect(res.status).toBe(200);
    });
  });
  ...
  describe('wrong token is rejected', () => {
    ...
    it('GET /api/v1/orchestrator/conversations returns 401 with wrong Bearer token', async () => {
      const res = await app.request('/api/v1/orchestrator/conversations', {
        headers: { Authorization: WRONG_AUTH },
      });
      expect(res.status).toBe(401);
    ...
    it('GET /api/v1/vault/credentials returns 401 with wrong Bearer token', async () => {
      const res = await app.request('/api/v1/vault/credentials', {
        headers: { Authorization: WRONG_AUTH },
      });
      expect(res.status).toBe(401);
    ...
    it('GET /api/v1/connectors returns 401 with wrong Bearer token', async () => {
      const res = await app.request('/api/v1/connectors', {
        headers: { Authorization: WRONG_AUTH },
      });
      expect(res.status).toBe(401);
    ...
    it('POST /api/v1/agents/browser/generate returns 401 with wrong Bearer token', async () => {
      const res = await app.request('/api/v1/agents/browser/generate', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ prompt: 'generate a test for login flow' }),
      });
      expect(res.status).toBe(401);
      const body = (await res.json()) as Record<string, string>;
      expect(body['error']).toBe('Unauthorized');

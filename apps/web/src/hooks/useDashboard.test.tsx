...
describe('dashboard hooks', () => {
  ...
  it('useRunDetail does not update state after unmount', async () => {
    ...
    const api = createApi({
      getRun: vi.fn(
        () =>
          new Promise<{ id: string; status: string; startedAt: string }>((res) => {
            resolve = res;
          }),
      ),
    });
  ...
  it('useAnalytics does not update state after unmount', async () => {
    ...
    const api = createApi({
      getAnalyticsSummary: vi.fn(
        () =>
          new Promise<{ totalRuns: number; passRate: number; avgDurationMs: number | null }>(
            (res) => {
              resolve = res;
            },
          ),
  ...
  it('useQuarantine does not update state after unmount', async () => {
    ...
    const api = createApi({
      getQuarantine: vi.fn(
        () =>
          new Promise<import('../lib/api.js').QuarantineEntry[]>((res) => {
            resolve = res;
          }),
      ),
    });

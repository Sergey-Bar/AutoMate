...
describe('GlobalCommandPalette', () => {
  ...
  it('nav-automate action calls navigate with /automate', () => {
    render(<ThemeProvider><GlobalCommandPalette /></ThemeProvider>);
    const action = commandStore.getActions().find(a => a.id === 'nav-automate');
    expect(action).toBeDefined();
    act(() => { action!.onSelect(); });
    expect(mockNavigate).toHaveBeenCalledWith({ to: '/automate' });
  });

  it('nav-webwright action calls navigate with /webwright', () => {
    render(<ThemeProvider><GlobalCommandPalette /></ThemeProvider>);
    const action = commandStore.getActions().find(a => a.id === 'nav-webwright');
    expect(action).toBeDefined();
    act(() => { action!.onSelect(); });
    expect(mockNavigate).toHaveBeenCalledWith({ to: '/webwright' });
  });

  it('settings-open action calls navigate with /settings', () => {
    render(<ThemeProvider><GlobalCommandPalette /></ThemeProvider>);
    const action = commandStore.getActions().find(a => a.id === 'settings-open');
    expect(action).toBeDefined();
    act(() => { action!.onSelect(); });

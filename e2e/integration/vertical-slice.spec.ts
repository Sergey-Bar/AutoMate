...
async function waitForRunInApi(
  ...
  await expect.poll(
    async () => {
      const res = await request.get(`${API_BASE}/api/v1/runs`);
...
test('vertical slice — reporter event reaches run list (API)', async ({ request }) => {
  ...
  const getRes = await request.get(`${API_BASE}/api/v1/runs`);
...
test('vertical slice — invalid event rejected, no run created (API negative)', async ({ request }) => {
...
test('vertical slice — SSE live update: run:end changes status to passed without reload (T29)', async ({ page, request }) => {

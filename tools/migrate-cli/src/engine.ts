...
export function createPlan(input: { source: string; sourceId: string; sourceCommit: string; targetCommit: string }) {
...
export function validatePlan(plan: ReturnType<typeof createPlan>): void {
  ...
  if (!/^[a-f0-9]{64}$/.test(plan.catalogFingerprint)) throw new Error('Invalid catalog fingerprint');
  if (plan.tables.some((table) => table.disposition === 'blocked')) throw new Error('Plan contains blocked tables');
